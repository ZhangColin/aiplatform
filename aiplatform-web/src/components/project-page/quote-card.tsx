 ...
 * 收尾卡同族。卡载事件（报价已出；改价「报价已更新」归 #204）+ 订单引用——金额/
 * 备注/状态渲染时取订单当前态（useOrder 详情查询，待报价/已报价态轮询现成），
 * 卡不冻结金额：已支付/已取消/已归档转态由同一视镜自然呈现，不残留可支付假象。
 * 卡内即支付闭环（复用 mock 支付 mutation 与确认弹窗）；订单 tab 保留完整详情
 * 与改价历史的家（「查看订单详情」跳转回调归装配层 openOutputsTo）。
 */
export function QuoteCard({
  ...
  orderId: string;
  /** 事件类型（quoted=报价已出；改价事件归 #204，未知值兜底同首报）。 */
  event: string;
