...
/**
 * 智能体模型引用：解析 {@code provider:modelId} 模型串，provider 白名单控制平台
 * 实际放开的模型提供方（AgentScope 模型扩展侧能力更宽，平台按需逐个放开——当前仅
 * deepseek，gemini 等后续加白）。
 *
...
public record ModelRef(String provider, String modelId) {
    ...
    private static IllegalArgumentException invalid(String modelString) {
        return new IllegalArgumentException(
                "不支持的模型串（应为 provider:model，当前支持 deepseek）: " + modelString);
    }
