...
/**
 * 一句话建项目（issue #39，spec 0002 §3.1）：POST /api/projects → 200 返回
 * {project, runId, accepted}（异步起 BA 运行，runId 即该运行全部 agent 流事件的
 * 锚，挂 /api/agent-events?runId=）。成功失效项目域（列表 / sidebar 徽章随建
 * 刷新）；直进工作台由调用侧按 project.id 导航——工作台按 projectId 挂 agent 流
