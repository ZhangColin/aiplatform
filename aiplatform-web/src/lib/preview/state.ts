 ...
 * generated，与 SSE 会话态无关——刷新/回访后档位仍正确）+ 修正轨会话信号
 * （coderStatus——更新中/修正失败，更新轨 RestartFix 现状不动）+ 预览查询结果 +
 ...
 * 出口；无推倒重来按钮（重走对话提意见改 PRD）。修正轮失败仍归「重新修改」
 * （更新轨，现状不动）。</p>
 */
...
export type PanelNotice = {
  ...
  text: string;
  /**
   * 恢复入口（#222 单出口）：resume = 继续生成（生成中断/从未生成——断点续跑或
   * 计划重派）/ refix = 重新修改（修正轮失败，更新轨现状不动）。
   */
  recovery?: "resume" | "refix";
};
...
export type SystemPanelPhase =
  ...
  /** 生成中断（无页面）：中断提示 +「继续生成」入口；修正轮失败给「重新修改」。 */
  | { kind: "failed"; text: string; recovery?: "resume" | "refix" }
 ...
 * 机制（生成中/中断——阶段 0 收口后应用可能已在跑/已生成）；修正轨会话信号在场
 * 同样启动（更新轨现状不动）。
 */
...
export function systemPanelPhase(input: {
  ...
  /** 本会话编码 run 状态（修正轨信号——更新中/修正失败；生成档位不依赖它）。 */
...
/**
 * 失败终态文案与恢复入口（占位与页面轻提示同源）：已生成（投影）→ 修正口径
 * 「重新修改」（更新轨 RestartFix 现状不动）；否则生成中断口径「继续生成」。
 */
