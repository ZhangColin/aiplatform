...
/** tone → 图标（active 转圈 / waiting 时钟 / settled 对勾）。 */
 ...
 * 开口或发送重置后）不渲染；失败链停在事发阶段（error 帧另行呈现）。
 */
export function DispatchStageBar({ projectId }: { projectId: string }) {
  ...
  return (
    <div
      className="mb-2 flex items-center justify-center gap-2 rounded-lg border bg-muted/40 px-3 py-1.5 text-xs text-muted-foreground"
