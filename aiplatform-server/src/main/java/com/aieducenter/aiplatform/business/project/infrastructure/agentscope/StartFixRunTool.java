 ...
 * 编码智能体可执行的修正任务（task）即派一场修正 run（复用 coder 会话与同工作区）。
 *
...
public class StartFixRunTool extends ToolBase {
    ...
    public StartFixRunTool(String workspaceId, IterationAppService iterationAppService) {
        super(ToolBase.builder()
                ...
                .description("派修正任务给编码智能体（系统已生成后，用户在指令区对系统提意见时调用）。"
                        + "task 传给编码智能体的修正说明：具体、可执行（要改哪里、改成什么样），"
    ...
    public Mono<ToolResultBlock> callAsync(ToolCallParam param) {
        Object task = param.getInput() != null ? param.getInput().get(TASK_KEY) : null;
        if (task == null || String.valueOf(task).isBlank()) {
            return Mono.just(ToolResultBlock.error("task 不能为空：传具体可执行的修正任务说明"));
        }
        try {
            IterationAppService.FixDispatch dispatch = iterationAppService
                    .startFixRunByWorkspace(workspaceId, String.valueOf(task));
            if (dispatch.queued()) {
