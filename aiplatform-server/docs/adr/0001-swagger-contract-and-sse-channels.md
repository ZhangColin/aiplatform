...
- 错误码 `{CONTEXT}_{NNN}`（编写规范既定），前缀注册表：`WSP_`（base.workspace）/ `KNW_`（base.knowledge）/ `PRJ_`（business.project）/ `METER_`（base.metering）/ `IDN_`（business.identity）——`AGT_`/`CHAT_` 随智能体层拆除注销（2026-08-31 平台重定义）；通用错误复用 `BaseCodeMessage`，不自造。新 BC 建立即注册前缀。
...
- **修订（权限确认端到端 · [票 #83](https://github.com/ZhangColin/aiplatform/issues/83)，2026-09-05）**：挂起事件按分诊拆两族——ask_user 提问 → `question-raised`（问答卡，答复文本）；需批准的工具操作 → `permission-required`（确认卡长在工作消息流，批准/拒绝布尔位）+ 落定 `permission-resolved`（确认卡转已批/已拒，重放面）。**作答通道分家**：问答作答 `POST /api/projects/{id}/questions/{qid}/answer` 与权限作答 `POST /api/projects/{id}/permissions/{ref}/answer` 互不串扰——权限作答不回传待确认工具清单（恢复私货在平台侧进程内会合点，runId 仅作串卡校验），过期/已落定 409 PRJ_027。生产触发面 = 平台侧 `command` 工具对破坏性命令自检 ASK（替位内核 shell：内核 shell 非 ToolBase 引擎拦不住，注册名 execute 也进不了部件播报表）。**〔已废止——[ADR-0020](../../../docs/adr/0020-permission-transparency-and-generation-resume.md)（票 [#219](https://github.com/ZhangColin/aiplatform/issues/219)，2026-09-20）：权限确认机制连代码带概念全删（确认卡、作答端点、替位 shell、超时机器），破坏性命令直通、过程经动作卡透明可见；ask_user 问答为唯一挂起作答面〕**
