...
- 状态管理三分法：REST 服务端状态 → TanStack Query（key 工厂 `src/lib/api/keys.ts`；SSE 事件以 invalidate 为主，失效注册表在 `src/lib/sse/bridge.ts`，ADR 0003；载荷展示例外由消费切片随用随登）；流式/纯 UI 状态 → Zustand store，放 `src/lib/store/`（SSE 相关三 store：`sse-status.ts` 连接状态、`agent-streams.ts` agent 流分段、`chat.ts` 指令区对话面——桥为唯一事件写入方）；一次性局部状态 → 组件 state
