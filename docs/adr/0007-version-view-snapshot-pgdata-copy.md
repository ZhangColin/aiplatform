...
- **实施配方**（#92 照单开工，无残留设计不确定点；开工顺序依 #75 ⑥ 实施序——#91 容器内 git 地基先行，配方第 4 步的当时代码来源由其供给）：
  ...
  4. 当时代码来源 = 容器内 git（#91）：检出当轮 commit 到快照容器本地目录跑；`node_modules` 落副本本地、不与主容器工作树互踩（卷 `:ro` 天然隔离）。**v1 收口**：依赖复用当前依赖（symlink 主容器 `node_modules`），不逐 ref 独立安装（`npm install` 慢且离线易脆）——历史代码跑在现在依赖上，依赖保真度见下方已知限制；
...
- **已知限制**（票面三问逐项）：
  ...
  - **依赖保真度**：v1 快照复用主容器当前 `node_modules`（symlink），不逐 ref 独立安装——历史代码跑在现在依赖上，依赖漂移（删包/升级）时历史代码起服可能挂或行为漂移。v1 依赖非交付物、历史代码多为静态/轻依赖，「查看当时」以代码面为主；逼近真实依赖漂移影响时再上逐 ref 独立安装（触发器备案）。
  - **孤儿清扫**：快照容器的在途注册表为进程内状态，平台重启即丢账、在途查看会话成孤儿——启动期按命名扫清全部 `ws-*-snap-*` 容器兜底（「不留孤儿容器」验收的兜底面）；正常路径靠关闭动作 `docker rm -f` 与工作区销毁级联。
