...
export interface paths {
    ...
    "/api/projects/{id}/files": {
        ...
        /**
         * 项目文件树（交付文件只读浏览）
         * @description 交付文件视图 = 项目 dev 工作区剔除非交付物（data/、.platform/、node_modules/ 与 .env——与源码包同口径）后的文件清单：[{path, size}]，path 为工作区相对路径、按路径稳定排序，只列文件（目录由前端按路径段合成）。直读工作区实时状态——生成/更新 run 完成后即反映最新文件长出。项目不存在 404 PRJ_001
         */
