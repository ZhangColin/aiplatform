...
public class ProjectController {
    ...
    @Operation(summary = "重新发起首次生成（失败兜底）",
            description = "生成无门自动发起（#101）后，主智能体产出 PRD 即平台自动派首次生成"
                    + " run，本端点退为失败兜底——run-failed 后项目仍「未生成」，系统面板"
                    + "「重新发起」重发此端点再触发。纯动作无门——PRD 已产出即可发起"
                    + "（待定项未清也可）。切片计划与片进度落生成轨道表（#220：计划跟 PRD"
                    + " 版本走，重发沿用表内现行计划——已收口片状态保留）；计划缺失（或"
                    + " PRD 已修订致旧计划过期）时不造假计划——平台重派主智能体按 PRD 补产"
                    + "切片计划，本端点返回补产轮 runId（主智能体对话轮，补产收口自动再派"
                    + "生成；同一 PRD 版本已补产过则 409 PRJ_017——重复触发口径，"
                    + "用户重提意见即兜底）。"
                    ...
                    + " agent=executor 起工作消息）。失败自动静默重试有限次"
