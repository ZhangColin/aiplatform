 ...
 * 待确认工具清单（question-raised 事件 {@code data.toolCalls} 原样回传，恢复私货从
 * 项目侧事实重建、不信前端）+ 用户答复文本（单选 label / 多选 label 拼接 /
 * 自由输入，可与已勾选合并——拼接归前端）。
 *
 * <p>路径 {@code /questions/{qid}/answer} 的 qid = 挂起事件 {@code engineRef}
 * （引擎侧请求 id，续跑批复的锚）。</p>
 *
 * @param runId     挂起轮的运行标识
 * @param toolCalls 待确认工具清单（{id, name, input}，挂起事件原样）
 * @param answer    用户答复文本
 */
public record AnswerQuestionCommand(
    ...
    /**
     * 待确认工具最小面（挂起事件 {@code data.toolCalls} 元素形状原样回传）。
     */
    public record ToolCall(
        ...
        /** 回挂起事件元素形状（{id, name, input}，null 兜空串/空 map——续跑重建口径）。 */
