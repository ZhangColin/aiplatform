...
/**
 * savePrd 修订事实登记（#52 交接物补齐）：主智能体对「PRD 改没改、改了什么」的判定
 * 从工具调用事实观测（判定契约铁律：不新增自报面）——工具执行体把必传的 summary
 * 落事实于此，意见轮收口（{@link MainAgentAppService}）消费入交接物。进程内
 * 事实（run 无表口径）：重启即清，与意见锚同取舍；key = 工作区（一个项目一个
 * 主智能体会话，事实即项目当下最新一次修订说明）。
 *
 * <p><b>生命周期</b>（与意见锚 {@code opinionExchanges} 同步）：意见轮起跑清残留
