...
- 状态管理三分法：REST 服务端状态 → TanStack Query（key 工厂 `src/lib/api/keys.ts`；SSE 事件以 invalidate 为主，失效注册表在 `src/lib/sse/bridge.ts`，ADR 0003；载荷展示例外由消费切片随用随登）；流式/纯 UI 状态 → Zustand store，放 `src/lib/store/`（SSE 相关 store：`sse-status.ts` 连接状态、`chat.ts` 对话面、`work-message.ts` 工作消息（编码 run 生长中的过程呈现，#81）、`generation.ts` 生成面、`prd-notices.ts` PRD 通告——桥为唯一事件写入方，发送侧乐观写入同 chat 先例；#82 旧族退役后旧 store（直播/派发阶段/agent-streams 分段）连代码带词条已删；`agent-runs.ts` 运行注册表（顶栏 LIVE 脉冲锚）亦已退役删除）；一次性局部状态 → 组件 state
