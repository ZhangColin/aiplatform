...
| type | payload 字段 | 说明 |
|---|---|---|
| `part-text` | `projectId` `runId` `sessionId` `engine` `text` | 解说文本部件：`text` 为**完整段非增量**——服务端逐段成型（句读 / 文本块变 / 步骤与动作边界 / 长度上限切段），run 收口事件前出尾段 |
