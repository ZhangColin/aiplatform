 ...
 * 超限终态恢复出口；#79 起为成果区「系统」tab）：恒为预览的容器。门禁解除——
 * run 开始（含发起成功的乐观登记）即取预览地址并挂机制，不等 run-finish 纪元；
 * 后端探活通过才返回 URL，有 URL 即上真页面（空白页可接受）。空态两档（推导
 * 归 lib/preview/state 纯函数，本组件只呈现）：无应用 = 占位随直播事件推进的
 * 步骤提示（自述优先、动作摘要兜底，无信号「正在初始化」）；有应用且 run 中 =
 * 保留页面 + 「更新中」轻提示（生长期与修正期同一套）。跨会话与重试不闪断：
 * 有 URL 就不退占位；run 收口纪元驱动 iframe 重挂（url+epoch 为 key）；超限
 * 终态给人工兜底入口——从未生成「重新发起」、修正轮「重新修改」，正常态全无。
 */
