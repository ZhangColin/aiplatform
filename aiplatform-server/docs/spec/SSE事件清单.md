...
>
> **双发射过渡期（#77–#82）**：通道二正处于新旧并行——parts 部件事件族（正本）与旧族事件（[过渡期旧族](#过渡期并行发射的旧族)）同时发射、语义各自不变；前端已于 #81 切新（消费 part-* 呈现生长中的工作消息，live-* 旧族前端已过滤不渲染），旧族由收缩票（#82）全量退役。两侧契约均以本文为唯一正本。
...
双发射过渡期（#77 起）继续发射的旧族事件——**语义与 #77 前完全不变**；前端已于 #81 切新（生长中的工作消息消费 part-*，本节旧族前端已过滤不渲染），由 #82 全量退役（一次动完，不设替代阶段事件）：
