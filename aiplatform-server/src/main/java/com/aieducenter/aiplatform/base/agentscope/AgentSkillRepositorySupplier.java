 ...
 * </b>发放技能仓库（技能来源三途：内置 / 安装 / 自产；#249 起合成「内置 classpath
 * ∪ 职能槽位已指派且启用的库技能」，见 {@code ProfileSkillRepositorySupplier}）；
 * 配置语境为空或未知时返回空集——无技能挂载即框架不注入 {@code
 * <available_skills>}（无挂载不注入）。工厂构建 agent 时取用（每次调用返回同一批
 * 实例，仓库通常长生命周期——仓库内容可动态查库，实例长寿命不等于面定格，
 * ADR-0021 生效语义）。
 *
