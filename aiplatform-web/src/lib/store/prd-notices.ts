...
export const usePrdNoticesStore = create<PrdNoticesState>((set) => ({
  ...
  notePrdWritten: (projectId) =>
    set((state) =>
      state.seen[projectId]
        ? { pending: { ...state.pending, [projectId]: true } }
...
/** 项目是否有未认领的 PRD 修订（胶囊 /「已更新」标记共用判读）。 */
export function hasPrdUpdate(
  state: Pick<PrdNoticesState, "pending">,
  projectId: string,
): boolean {
  return state.pending[projectId] !== undefined;
}
