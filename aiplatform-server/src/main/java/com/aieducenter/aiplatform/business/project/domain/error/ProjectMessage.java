...
public enum ProjectMessage implements CodeMessage {
    ...
    /** 更新任务前置事实守卫：系统从未生成（迭代在首次生成完成后才开始）。 */
    ...
    /** 更新恢复出口守卫（#48）：更新在途（进行中/排队中）无手动恢复面——链自动在跑。 */
    FIX_RESTART_IN_FLIGHT(409, "PRJ_025", "更新正在进行中，无需手动恢复"),

    /** 更新恢复出口守卫（#48）：无超限终态账（未派过/已成功收工/重启丢账），指路重提意见。 */
    FIX_RESTART_UNAVAILABLE(409, "PRJ_026", "没有可恢复的更新，请在对话区重新提意见"),

    /** PRJ_027 已随权限确认机制删除（#219 透明面化）——序号不复用，登记制不重排。 */
