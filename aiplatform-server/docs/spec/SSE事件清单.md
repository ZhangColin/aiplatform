...
- `ts` 为 ISO-8601 信封时间戳：前端唯一消费 = run 级「已运行」时钟锚（[#225](https://github.com/ZhangColin/aiplatform/issues/225)——run-start 起锚、run-finish / run-failed 定格，ADR-0010 窄修订），其余消费不依赖。
...
| `order-status-changed` | `projectId` `orderId` `status` `statusName` | `{"projectId":"a1b2c3d4","orderId":"900123","status":2,"statusName":"已报价"}`（订单状态变化后发射：下单（待报价）/首次报价（已报价）/取消/支付完成/归档（已归档）各发一次——支付与归档分两发，归档失败只发「已支付」，改价不换状态、不发本事件（改价信号 = `order-repriced`）；发射方 = order 上下文 `OrderEventTypes`，副作用真实落定后（支付路径在支付/归档各自事务提交后）。前端消费 = toast（点击直达项目页）+ 失效订单/项目域重查；状态 code 为 OrderStatus Integer：1=待报价 2=已报价 3=已支付 4=已归档 5=已取消（已支付为真实态，支付落定后发）。[#30](https://github.com/ZhangColin/aiplatform/issues/30) 新增，[#39](https://github.com/ZhangColin/aiplatform-server/issues/39) 支付原子化修订，[#204](https://github.com/ZhangColin/aiplatform/issues/204) 改价拆出） |
| `order-repriced` | `projectId` `orderId` `status` `statusName` | `{"projectId":"a1b2c3d4","orderId":"900123","status":2,"statusName":"已报价"}`（订单已改价：已报价态改价落定后发射——改价不换状态，本事件即改价信号（[#204](https://github.com/ZhangColin/aiplatform/issues/204) 改价入流，推翻「改价不换状态不发」的静默，ADR-0017）。发射时序 = 先落「报价已更新」对话卡后发（信号触发前端重查时卡已在库，#203 同款）；载荷与 `order-status-changed` 同字段同量级（`status` 恒 2=已报价），**不含金额与备注**。前端消费 = 失效订单/项目/对话史域重查——在场项目页「报价已更新」卡实时入流、历史报价卡经订单重查显当前价（视镜）；toast 分流（他页路标）归 #206。[#204](https://github.com/ZhangColin/aiplatform/issues/204) 新增） |
...
**消息 = 有序部件集合（parts）**（六源行业同构；词根基准 = agentscope 原生 part 族——文本/推理/工具调用请求/文件，mapper 保持薄翻译层，不强套某家词表）。run 进行中 = 对话区内一条**生长中的工作消息**（解说文本部件 + 工具动作部件，带状态；部件按序竖排，无步骤分组），run 收口 = 消息定格。部件事件（`part-*`）由 base.agentscope 的部件映射表（`AgentscopePartsMapper`）从同一 AgentScope 事件流产出，全事件流恒挂（不限编码 run）——例外：`part-check` 由平台侧收口判据核验产出（自检是平台事实，不经引擎 mapper，[#85](https://github.com/ZhangColin/aiplatform/issues/85)）；`part-plan` 是快照式部件（#236 run 级步骤清单——整表替换、不进部件流水序，工作卡计划区渲染）；思考与代码补丁不进部件（思考与代码流不播——CONTEXT.md「智能体事件」）。
...
| type | payload 字段 | 说明 |
|---|---|---|
| `part-text` | `projectId` `runId` `sessionId` `engine` `source`（可缺省） `text` | 解说文本部件：`text` 为**完整段非增量**——服务端逐段成型（句读 / 文本块变 / 动作边界 / 来源切换 / 长度上限切段），run 收口事件前出尾段。机器语法守卫（[#234](https://github.com/ZhangColin/aiplatform/issues/234) 叙事段堵漏）：模型以原生工具参数语法（DSML 工具调用标记族，全角竖线连打变体同族）直接发射、引擎未识别为工具调用的文本**识别即丢弃**——不成段、不转译、不挪位到其他面（无闭合 = 脱轨，吞至 run 尾；跨增量 split 的标记头按住不成段）；吞段开始同步产 `part-signal` 脱轨信号（见下行） |
| `part-signal` | `projectId` `runId` `sessionId` `engine` `source`（可缺省） `signal` | 脱轨信号部件（[#240](https://github.com/ZhangColin/aiplatform/issues/240) 机器语法吞段的人话留痕）：守卫识别到机器语法段（标记起点）即发射——脱轨 = 模型以为调了工具、实际什么都没跑（文本轮即终），不静默。载荷只有发生事实：`signal` 为封闭词表，v1 唯一值 `derailed`；**不携带任何原文**（#234 口径维持——界面任何位置不见工具语法原文，原文出口 = 后端 trace 日志）。呈现 = 活性行脱轨变体（#235 变体族加脱轨变体，一句定型文案）——**静态面无痕**（正文投影滤除）、**不伪造动作**（不产生 part-action、命令滚动行不出未执行命令）。频次：每次吞段一句（标记起点判定，吞段中的后续增量不刷），天然低频 |
| `part-action` | `projectId` `runId` `sessionId` `engine` `source`（可缺省） `toolCallId` `toolName` `state` `label` `error`（可缺省） | 工具动作部件（动作卡）：**开始/进行中/完成/失败全生命周期**——动作一开始即出事件，同一动作以 `toolCallId` 锚定跨状态更新。`state` ∈ `started`（模型发起工具调用，参数在途）/ `running`（参数落定、工具执行中）/ `completed`（结果成功）/ `failed`（结果出错/被拒/中断——动作层状态，run 层唯一失败终态仍是 run-failed）。`label` = 动作对象短语（人话行，无时态——时态由 state 表达；started 时参数在途为通用对象如「编写【代码文件】」，running 起解析参数为具体对象如「编写【订单管理】」，终态复述不闪换）；**execute 的 label = 命令原文首行**（[#228](https://github.com/ZhangColin/aiplatform/issues/228) 直播行动态化：剥 `bash -c` / `sh -c` 壳与 `cd … &&` 前缀后裸显、单行定宽 80 截断〔含省略号，走查票校准〕；参数在途 / 解析不出回落「运行命令」，滚动行成为 claude code / replit 式 live tail）。`error`（[#229](https://github.com/ZhangColin/aiplatform/issues/229) 失败留痕）**仅 `state=failed` 时携带**——错误/stderr 首行截断（工具结果文本经增量累积后取首行，与 label 各管各的截断额度：定宽 120 含省略号，走查票校准）；结果文本为空 / 首行空白不携带——失败红行＝label＋error，排障不进容器即可初判原因。播报工具为封闭表：write_file / edit_file / execute（内核 shell 注册名——命令直通后破坏性命令照常播报，[#219](https://github.com/ZhangColin/aiplatform/issues/219)）——读类工具不进部件（对客户是噪音） |
...
| `part-plan` | `projectId` `runId` `sessionId` `engine` `source`（可缺省） `steps` | 步骤清单部件（[#236](https://github.com/ZhangColin/aiplatform/issues/236)）：run 级步骤清单的**全量快照**——agent 调 `update_plan` 工具自产计划（每 run 开工先调一次、随推进再调），由部件映射表从工具参数增量解析产出（参数落定点 ToolCallEnd 出事件）。`steps` = `[{ id, title, state }]` 全量数组（id 稳定跨调用不变——就地整表更新的锚；title 用户语言一句话；`state` ∈ `pending`（○ 待做）/ `in_progress`（● 当前）/ `completed`（✓ 已收口），**不设 ✗ 态**——失败留痕归 `part-action` 与收尾卡）。**快照式**：执行中再调即新事件整表替换（已收口步骤不可变＝提示词纪律，平台 v1 不强制校验；消费端不合并）；**不走动作行、不留动作痕**（计划变化不产生动作部件、不单独播报）；断线补发以**最后快照**为准（重放按事件序 + 事件 id 去重）；**不落库**（当次会话定格留驻、刷新不回显——过程明细口径维持）；agent 不调用则无事件（界面无清单区域，解说兜底）。契约与来源解耦：不携带计划来源（v1 run 执行体提示词直产，skill 线接入时再议标注） |
...
> **来源归属（#95 委派位）**：委派子智能体（如自测）转发进父流的过程事件带 `source` 字段（值 = 子智能体声明名，引擎 source 路径的末段）；run 执行体自身产出的过程事件**不携带** `source`（缺省 = 执行体——用户面仍无角色标签，source 只用于过程呈现归属）。携带范围：引擎透传事件（`text`/`reasoning`/`tool`/`step-*`，payload 顶层）与消息部件事件（`part-text`/`part-signal`/`part-action`/`part-plan`——后者是事件归属非计划来源，见该行注）。生命周期事件（`run-start`/`run-finish` 等）恒为执行体层级，不带 source；`part-check` 为平台侧产出，也不带。
  ...
  durationBreakdown: {             # 阶段耗时分布（#111——平台分析口径，前端不渲染；
    ...
    attempts: [                    #   逐尝试分布（静默重试代价可归因）——attempt / durationMs
      { attempt: 1, durationMs: 61000, llmMs: 52000, toolsMs: {...}, selfTestMs: 9000 }
                                   ...
                                   #   （阶段耗时事实随异常弃置）、墙钟照记
    # 一致性口径：桶计 + 未归因差值（平台管道 / 判据未过的核验）≈ durationMs——
...
> **智能体事件桥**：AgentScope HarnessAgent 的事件经映射表翻译走同一通道同一信封——`engine=agentscope`，事件序 `run-start → 过程事件（透传 + 部件并行）→ run-finish/error`。透传映射表单点 = `AgentscopeEventMapper`；部件映射表单点 = `AgentscopePartsMapper`（解说切段与动作行的生产内核由 `NarrationSegments` / `ToolActionLines` 承载）。挂起（`RequireUserConfirmEvent`）唯一形态 = ask_user 提问 → `question-raised`（问答卡；`data.questions` 为前端投影：`[{header, question, multiple, custom(恒 true), options[{label}]}]`，`summary` 取问题文本；权限确认机制已随透明面化删除，[#219](https://github.com/ZhangColin/aiplatform/issues/219)——破坏性命令直通不挂起，过程经动作卡播报）；作答走问答作答通道（`POST /api/projects/{id}/questions/{qid}/answer`，答复文本）。挂起轮不发 `run-finish`；答复续跑归业务编排（从项目侧事实重建恢复私货 + 挂起事件 `data.toolCalls` 重建 ConfirmResult）；会话状态落 PostgreSQL（`cat_agent_state` 承载全部智能体会话），平台重启后按会话标识恢复续跑。
