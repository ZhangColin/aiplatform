 ...
 * 三分法）：按项目累积「发送前」的圈注条目（预览上点选/圈选/评论的产物，发往
 * 发送框附件区）。系统面板（收预览 iframe 的 postMessage 锚）写入、发送框消费
 * （chip 呈现 + 删改）、发送即清。发送前可删改（remove / updateNote）。
 */

/** 一条圈注条目（draft 载荷 + 本地 id——React key 与删改锚）。 */
...
export type AnnotationState = {
  ...
  /** 发送前改评语（评语可选——删除即删改之「改」）。 */
  updateNote: (projectId: string, id: string, note: string) => void;
  /** 发送即清（附件随消息发出，不再滞留）。 */
...
export const useAnnotationStore = create<AnnotationState>((set) => ({
  ...
  updateNote: (projectId, id, note) =>
    set((state) => ({
      annotations: {
        ...state.annotations,
        [projectId]: (state.annotations[projectId] ?? []).map((item) =>
